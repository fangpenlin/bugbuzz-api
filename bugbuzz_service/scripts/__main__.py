...
from bugbuzz_service import scripts
from bugbuzz_service.settings import default_settings
...
DEFAULT_CONFIG_FILE = '/etc/bugbuzz/bugbuzz.ini'
...
@click.option(
    '-c', '--conf-file',
    type=str,
    help='configuration FILE, defaults to BUGBUZZ_CONF, use - for defaults',
    default=os.getenv('BUGBUZZ_CONF', DEFAULT_CONFIG_FILE),
)
@click.option(
    '-v', '--version',
    is_flag=True,
    help='print bugbuzz version',
)
@click.pass_context
def cli(ctx, log_level, conf_file, version):
    ...
    if os.path.exists(conf_file):
        cfg_settings = get_appsettings(conf_file, name='bugbuzz_app')
